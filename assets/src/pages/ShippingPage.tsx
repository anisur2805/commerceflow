  ...
  const remove = useCallback(
    (rule: ShippingRule) => {
      apiFetch({
        ...
        .then(() => {
          addToast(__("Shipping rule deleted.", "commerceflow"), "success");
          load();
        })
        .catch((err: Error) =>
          addToast(
  ...
  if (error) {
    return (
      <Notice status="error" isDismissible={false}>
        {__("Failed to load shipping rules:", "commerceflow")} {error}
      </Notice>
    );
  }
          ...
          {rules.length === 0 ? (
            ...
            >
              <thead>
                ...
                >
                  <th style={headCell}>{__("Priority", "commerceflow")}</th>
                  <th style={headCell}>{__("Name", "commerceflow")}</th>
                  <th style={headCell}>{__("Rate", "commerceflow")}</th>
                  <th style={headCell}>{__("Status", "commerceflow")}</th>
                  <th style={headCell}>{__("Actions", "commerceflow")}</th>
                </tr>
              </thead>
              <tbody>
                {rules.map((rule) => (
                  ...
                  >
                    <td style={bodyCell}>{rule.priority}</td>
                    <td style={bodyCell}>{rule.name}</td>
                    <td style={bodyCell}>
                      {rule.rate.label} ({rule.rate.cost})
                    </td>
                    <td style={bodyCell}>
                      {rule.enabled
                        ? __("Enabled", "commerceflow")
                        : __("Disabled", "commerceflow")}
                    </td>
                    <td style={bodyCell}>
                      <Button
                        variant="secondary"
...
function RuleEditor({
  ...
  const save = () => {
    ...
    apiFetch({ path, method: draft.id ? "PUT" : "POST", data: payload })
      .then(() => {
        addToast(__("Shipping rule saved.", "commerceflow"), "success");
        onSaved();
      })
      .catch((err: Error) => {
        addToast(err?.message ?? __("Save failed.", "commerceflow"), "error");
        setSaving(false);
      });
  };
    ...
    >
      ...
      >
        ...
        <TextControl
          ...
          value={sample.state}
          onChange={(value) => setSample((prev) => ({ ...prev, state: value }))}
        />
        <TextControl
      ...
      {result && (
        <p style={{ marginTop: "12px", fontSize: "14px" }}>
          {result.matched && result.rate
            ? `${__("Matched:", "commerceflow")} ${result.rate.name} — ${
                result.rate.label
              } (${result.rate.cost})`
