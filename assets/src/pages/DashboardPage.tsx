    ...
    <div>
      ...
      >
        ...
        >
          ...
          {isLoading ? (
            <p style={{ color: "#9ca3af" }}>{__("Loading…", "commerceflow")}</p>
          ) : (
      ...
      >
        <FulfillmentCard data={data?.fulfillment ?? []} loading={isLoading} />
        <AutomationQueueCard />
        <ShippingCard
...
function AutomationQueueCard() {
  ...
  if (error) {
    ...
    body = <p style={{ color: "#9ca3af" }}>{__("Loading…", "commerceflow")}</p>;
        ...
        <tbody>
          {data?.map((log) => (
            <tr key={log.id} style={{ borderBottom: "1px solid #f3f4f6" }}>
              ...
              >
                #{log.rule_id} · {log.trigger.replace(/_/g, " ")}
              </td>
              <td
                style={{
                  padding: "8px 4px",
                  fontSize: "12px",
                  textAlign: "right",
                  color: statusColor[log.status] ?? "#6b7280",
                  fontWeight: 600,
                }}
              >
...
function RevenueChart({
  ...
  return (
    ...
    >
      {data.map((day) => {
        const height = Math.max((day.revenue / maxRevenue) * barHeight, 2);
        return (
          <div
            key={day.date}
            style={{
