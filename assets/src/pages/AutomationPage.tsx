...
export function AutomationPage() {
  ...
  const handleSave = useCallback(() => {
    ...
    apiFetch<Rule>({ path, method, data: editing })
      .then(() => {
        setEditing(null);
        addToast(__("Rule saved successfully.", "commerceflow"), "success");
        setIsSaving(false);
        loadRules();
      })
      .catch((err: Error & { message?: string }) => {
        addToast(
          err?.message ?? __("Failed to save rule.", "commerceflow"),
          "error",
        );
        setIsSaving(false);
      });
  ...
  const handleDelete = useCallback(
    (id: number) => {
      apiFetch<{ deleted: boolean }>({
        ...
        .then(() => {
          addToast(__("Rule deleted.", "commerceflow"), "success");
          loadRules();
        })
        .catch((err: Error & { message?: string }) => {
          addToast(
            err?.message ?? __("Failed to delete rule.", "commerceflow"),
            "error",
          );
        });
    },
  ...
  const handleToggle = useCallback(
    (rule: Rule) => {
      ...
      apiFetch<Rule>({
        ...
        .catch((err: Error & { message?: string }) => {
          addToast(
            err?.message ?? __("Failed to toggle rule.", "commerceflow"),
            "error",
          );
        });
    },
  ...
  const handleDryRun = useCallback((id: number) => {
    ...
    apiFetch<unknown>({
      ...
      .catch((err: Error & { message?: string }) => {
        setDryRunResult({
          error: err?.message ?? __("Dry-run failed.", "commerceflow"),
        });
      });
  }, []);
  ...
  return (
    <div>
      ...
      >
        ...
        <Button variant="primary" onClick={() => setEditing(emptyRule())}>
    ...
    <div style={cardStyle}>
      ...
      >
        ...
        <div style={{ display: "flex", gap: "8px" }}>
          ...
          <Button variant="secondary" isSmall isDestructive onClick={onDelete}>
      ...
      >
        {rule.actions.length} {__("action(s)")} · {__("priority")}:{" "}
        {rule.priority}
      </div>
    </div>
  );
  ...
  return (
    ...
    >
      <div style={{ minWidth: "520px", maxWidth: "600px" }}>
        ...
        <h3 style={sectionTitle}>{__("Conditions", "commerceflow")}</h3>
        {rule.conditions.map((cond, idx) => (
          ...
          >
            <TextControl
              placeholder={__("field (e.g. total)", "commerceflow")}
              value={cond.field}
              onChange={(v) =>
                updateCondition(rule, idx, { field: v }, onChange)
              }
            />
            <SelectControl
              value={cond.operator}
              options={OPERATORS}
              onChange={(v) =>
                updateCondition(rule, idx, { operator: v }, onChange)
              }
            />
            <TextControl
              ...
              onChange={(v) =>
                updateCondition(rule, idx, { value: v }, onChange)
              }
            />
            <Button
              variant="secondary"
        ...
        {rule.actions.map((action, idx) => (
          <ActionEditor
            key={idx}
            action={action}
            onChange={(a) => updateAction(rule, idx, a, onChange)}
            onRemove={() =>
              onChange({
                ...rule,
        ...
        <Button
          ...
          onClick={() =>
            onChange({
              ...rule,
              actions: [...rule.actions, { type: "change_status", config: {} }],
            })
          }
        >
        ...
        <div style={{ marginTop: "24px" }}>
          <TextControl
            ...
            onChange={(v) => update({ priority: parseInt(v, 10) || 0 })}
          />
          <ToggleControl
  ...
  return (
    ...
    >
      ...
      >
        ...
        />
        <Button variant="secondary" isSmall isDestructive onClick={onRemove}>
          ×
        </Button>
      </div>
      ...
      {action.type === "add_order_note" && (
        <TextareaControl
          ...
          onChange={(v) => onChange({ ...action, config: { note: v } })}
        />
      )}
      ...
      {action.type === "call_webhook" && (
        <TextControl
          ...
          onChange={(v) => onChange({ ...action, config: { url: v } })}
        />
      )}
    </div>
  );
