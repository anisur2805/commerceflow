  ...
  const transition = useCallback(
    (order: Order, to: TransitionOption) => {
      ...
      apiFetch<Order>({
        ...
        .then(() => {
          addToast(__("Order status updated.", "commerceflow"), "success");
          setBusyId(null);
          load();
        })
        .catch((err: Error) => {
          addToast(
            err?.message ?? __("Transition failed.", "commerceflow"),
            "error",
          );
          setBusyId(null);
        });
          ...
          {orders.length === 0 ? (
            ...
            >
              <thead>
                ...
                >
                  <th style={headCell}>{__("Order", "commerceflow")}</th>
                  <th style={headCell}>{__("Customer", "commerceflow")}</th>
              ...
              <tbody>
                {orders.map((order) => (
                  ...
                  >
                    ...
                    <td style={bodyCell}>{order.customer || "—"}</td>
                    <td style={bodyCell}>{order.status_label}</td>
                    <td
                      style={{
                        ...bodyCell,
                        textAlign: "right",
                        ...
                        {order.allowed_transitions.length === 0 ? (
                          ...
                          order.allowed_transitions.map((option) => (
                            <Button
                              ...
                              onClick={() => transition(order, option)}
                            >
                              {option.label}
                            </Button>
                          ))
...
function TimelineModal({
          ...
          >
            <div
              style={{
                fontSize: "13px",
                fontWeight: 600,
                color: entry.source === "automation" ? "#7c3aed" : "#111827",
              }}
            >
              {entry.message}
            </div>
  ...
  return (
    <Modal
      title={`${__("Timeline — Order #", "commerceflow")}${order.number}`}
