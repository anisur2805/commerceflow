  ...
  useEffect(() => {
    apiFetch<Settings>({ path: "commerceflow/v1/settings" })
      ...
      .catch((err: Error & { message?: string }) => {
        setError(
          err?.message ?? __("Failed to load settings.", "commerceflow"),
        );
        setIsLoading(false);
      });
  }, []);
  ...
  const handleSave = useCallback(() => {
    ...
    apiFetch<Settings>({
      ...
      .then((result) => {
        ...
        addToast(__("Settings saved successfully.", "commerceflow"), "success");
        setIsSaving(false);
      })
      .catch((err: Error & { message?: string }) => {
        setNotice({
          type: "error",
          message:
            err?.message ?? __("Failed to save settings.", "commerceflow"),
        });
        addToast(
          err?.message ?? __("Failed to save settings.", "commerceflow"),
          "error",
