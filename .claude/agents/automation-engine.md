...
3. **Partial failure (FR-AUTO-6).** If action _n_ of _m_ fails, order state must not be left corrupt. Earlier actions stay intact, later actions handled per a documented continue/stop policy, failure logged with context.
4. **Dry-run (FR-AUTO-7).** A rule can be tested reporting what _would_ happen, writing zero order changes.
